        ...
        {nodes.map((node, i) => (
          <Fragment key={node.id}>
            ...
            >
              ...
              <p className="text-xs text-slate-400 leading-snug">{node.sublabel}</p>
              {node.highlight && (
          ...
          <>
            ...
            {HAS_API ? (
              ...
              >
                ...
                <p className="text-slate-400 text-sm mb-1">olmOCR → alt text → semantic HTML → axe-core → live-reviewer loop</p>
                <p className="text-slate-400 text-xs">Live runs take a few minutes and call real models.</p>
                <p className="text-slate-400 text-xs mt-3"><button type="button" className="text-teal-400 hover:text-teal-300 underline" onClick={(e) => { e.stopPropagation(); document.getElementById('pdf-upload')?.click(); }}>or click to upload from your computer</button></p>
              ...
              <div className="border-2 border-dashed border-slate-700 rounded-xl p-8 text-center">
                <Layers className="mx-auto mb-3 text-slate-400" size={28} />
                <p className="text-slate-300 font-medium mb-1">Replay a real benchmark run</p>
                <p className="text-slate-400 text-sm">olmOCR → alt text → semantic HTML → axe-core → live-reviewer loop.</p>
                <p className="text-slate-400 text-xs mt-1">Live PDF upload runs in self-hosted mode.</p>
              </div>
            )}
            <div>
          ...
          <div className="space-y-5">
            ...
            {isLive && !done && job?.status === 'running' && (
              <div className="flex items-center justify-between px-3 py-2 rounded-lg bg-slate-800/60 border border-slate-700/40 text-xs">
                ...
                {job?.page_count ? (
                  <span className="text-slate-400 font-mono shrink-0 ml-3">{job.page_count} page{job.page_count > 1 ? 's' : ''}</span>
                ) : null}
              </div>
            )}
                  ...
                  return (
                    <div key={s.id} className={`px-3 py-2 rounded-lg text-sm transition-all ${active ? 'bg-teal-400/10 border border-teal-400/20' : isDone ? 'opacity-60' : 'opacity-30'}`}>
                      <div className="flex items-center gap-3">
                        ...
                        <span className={isDone ? 'text-slate-300' : active ? 'text-teal-300' : 'text-slate-400'}>{s.label}</span>
            ...
            {inReviewRound && isLive && (
              <div className="border border-slate-700/60 rounded-xl p-3">
                <p className="text-xs text-slate-400 font-medium uppercase tracking-wider mb-2">Reviewing now</p>
                <div className="flex flex-wrap gap-2">
            ...
            {done && job?.reviewer_health && Object.keys(job.reviewer_health).length > 0 && (
              <div className="border border-slate-700/60 rounded-xl p-4">
                <p className="text-xs text-slate-400 font-medium uppercase tracking-wider mb-3">Peer reviewers</p>
                <div className="space-y-2">
                  {Object.entries(job.reviewer_health).map(([model, health]) => {
                    ...
                    const statusColor = isNotRun ? 'text-slate-400 italic' : (health.status === 'success' ? 'text-emerald-400' : 'text-rose-400');
            ...
            {hasResults && job && (
              <div className="border border-slate-700/60 rounded-xl p-4 space-y-3">
                <p className="text-xs text-slate-400 font-medium uppercase tracking-wider">Automated check coverage (axe-core)</p>
                <div className="flex items-center justify-around">
                  ...
                  <div className="text-center space-y-1">
                    ...
                    <p className="text-xs text-slate-400">{done ? 'Final' : 'Latest'}</p>
                ...
                {job.enhancements.length === 0 ? (
                  ...
                  job.enhancements.map((e, i) => (
                    <div key={i} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs border border-emerald-500/20 bg-emerald-500/5">
                      ...
                      <span className="ml-auto font-mono text-slate-400 truncate max-w-[35%]">{e.element_id}</span>
                    </div>
                  ))
                )}
              </div>
...
export default function App() {
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans antialiased">
      ...
      <header className="sticky top-0 z-50 border-b border-slate-800/80 bg-slate-950/90 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <a href="/" className="flex items-center gap-2.5 group" aria-label="HappyPDF home">
            ...
            <span className="text-[10px] font-mono text-slate-400 bg-slate-800 px-1.5 py-0.5 rounded ml-0.5">
              WCAG 2.2
            </span>
          </a>
      ...
      {/* ── Caveat banner ────────────────────────────────────────────────── */}
      <aside aria-label="Automated coverage disclaimer" className="bg-amber-500/10 border-y border-amber-500/20">
