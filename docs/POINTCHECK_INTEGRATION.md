...
Status: **Phase 1 implemented (July 16 2026)**; **Phase 2 implemented, STAGING ONLY (July 16 2026)**: prod promotion pending the maintenance window (`modal deploy src/modal_api.py`, then one prod e2e; the `alttext-judge` Modal app is already deployed and is only referenced by the new API code). Phase 2 notes: `modal/modal_alttext_judge.py` (Molmo-7B-D 4-bit on A10G, weights baked, `transformers==5.14.1` pinned) + `judge_alt_text_map()` in `build_syllabus_slice.py` + concurrent collection in `_live` (report-only `alt_text_review` block). Generation uses a MANUAL greedy decode loop with Molmo's legacy tuple cache, transformers' `generate()` is incompatible with this model's remote code (modern versions pre-inject a DynamicCache; Molmo's forward does `past_key_values[0][0].size(-2)` → `'list' object has no attribute 'size'`). Flag threshold calibrated at score ≤2 (the judge is strict: good alt scored 3, filename/wrong alt scored 1 on the Accessible University logo smoke test).

**Phase 3 implemented, STAGING ONLY (July 16 2026)**: same prod-promotion step as Phase 2. Implementation refines the original design: the HTML side needs no vision (it's structured: DOM counts via `fidelity_gate.html_inventory`); Molmo only inventories the rendered PDF pages (`judge_page_fidelity` in the alttext-judge app), which sidesteps the PDF-page-to-HTML-region alignment problem entirely. Comparison is loss-only with calibrated tolerances (`src/fidelity_gate.py::_compare`): off-by-one image counts are VLM noise; table findings need cross-page agreement; and **scanned documents suppress count comparisons** (calibration on navy_bulletin: the model counts figures inside a page scan while the HTML counts extracted rasters: different units; the critical `text_rendered_as_image` finding carries the message instead). The PDF-side GPU work starts at upload time so it overlaps extraction and the review rounds. Calibration: zero findings on the two clean benchmark docs; navy_bulletin produced exactly one finding; the TRUE-POSITIVE scanned-pages catch (pages 1-2 have no text layer). Staging e2e on navy_bulletin: fidelity block correct, and the Phase 2 judge + Phase 1 checks independently caught a real filename-alt failure (`syllabus_img_3.jpeg`) that axe scored 100% past. Phase 4 items remain proposed. Implementation note: the checks run via `pointcheck_score(html)`: a one-shot sync-Playwright run mirroring `loop.axe_score()`: called post-baseline and post-loop in `_live` rather than inside `_run_loop_inner`; the JS strings were copied verbatim, so no async bridge was needed at all. Validation: 3 new pytest cases (planted issues fire; clean document-shaped HTML is silent; web-noise prunes verified), zero false positives against all three `api/snapshots/` final_htmls (each 100% on axe), and a staging + prod e2e conversion with both blocks present and the pipeline unaffected.
