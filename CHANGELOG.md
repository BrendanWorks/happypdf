...
**Known model note:** transformers' `generate()` is incompatible with Molmo-7B-D's remote code on modern versions (it pre-injects a `DynamicCache` that Molmo's forward pass cannot read). The judge uses a manual greedy decode loop over Molmo's `forward()` with legacy tuple caches instead.
